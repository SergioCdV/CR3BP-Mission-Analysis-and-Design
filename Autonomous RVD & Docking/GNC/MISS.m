...
while ((GoOn) && (iter < maxIter))    
    ...
    switch (cost)
        case 'Position' 
            ...
            %Preallocation of the SMT 
            STM = zeros(3,3*impulses);
            
            %Compute the STM 
            for i = 1:length(times)
                aux = reshape(S(times(i),13:end), [length(r_t0) length(r_t0)]);      %STM evaluated at time tf
        ...
        case 'State' 
            ...
            %Preallocation of the SMT 
            STM = zeros(6,6*impulses);
            
            %Compute the STM 
            for i = 1:length(times)
                STM(:,1+6*(i-1):6*i) = reshape(S(times(i),13:end), [length(r_t0) length(r_t0)]);      %STM evaluated at time tf
            end      
    ...
    for i = 1:length(times)-1
        ...
        %Make the impulse
        S(times(i+1),10:12) = S(times(i+1),10:12) + dV(1+3*(i-1):3*i,:).';
    end

...
c = plot3(S_rc(:,1), S_rc(:,2), S_rc(:,3), 'k-*', 'MarkerIndices', 1:10:size(S_rc,1));
plot3(St(:,1)+St(:,7), St(:,2)+St(:,8), St(:,3)+St(:,9), 'r', 'LineWidth', 0.7); 
legend('Target orbit', 'Loiter orbit', 'Chaser motion', 'Location', 'northwest', 'AutoUpdate', 'off');
scatter3(St(end,1)+St(end,7), St(end,2)+St(end,8), St(end,3)+St(end,9), 'k', 'filled');
scatter3(St(1,1)+St(1,7), St(1,2)+St(1,8), St(1,3)+St(1,9), 'k', 'filled');
...
subplot(1,2,1)
hold on
plot(tspan_misg, S3(:,7)); 
plot(tspan_misg, S3(:,8)); 
plot(tspan_misg, S3(:,9)); 
hold off
xlabel('$t$');
ylabel('$\mathbf{\rho}$');
grid on;
...
subplot(1,2,2)
hold on
plot(tspan_misg, S3(:,10)); 
plot(tspan_misg, S3(:,11)); 
plot(tspan_misg, S3(:,12));
hold off
xlabel('$t$');
ylabel('$\dot{\mathbf{\rho}}$');
grid on;
...
figure 
stem(tspan_1, [sqrt(dot(u,u,1)) 0]*Vc, 'filled');
grid on; 
xlabel('$t$')
ylabel('$||\Delta\mathbf{V}||$')

figure 
plot(tspan_2, sqrt(dot(u_smc,u_smc,1))*(4*pi^2*Lem/Tc^2)*1e3);
grid on; 
xlabel('$t$')
ylabel('$||\mathbf{u}||$')

figure 
stem(tspan_misg, sqrt(dot(u_misg,u_misg,1))*Vc, 'filled');
grid on; 
xlabel('$t$')
ylabel('$||\Delta\mathbf{V}||$')

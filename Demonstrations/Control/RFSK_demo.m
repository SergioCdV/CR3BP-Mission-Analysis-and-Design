...
GNC.Control.RFSK.Q = 1e3*eye(2);                      % Penalty on the state error
GNC.Control.RFSK.M = eye(3);                          % Penalty on the control effort
...
% Error in time 
[e, merit] = figures_merit(tspan, St(:,n+1:2*n));

% Control law
[~, ~, u] = GNCc_handler(GNC, Staux1(:,1:n), Staux1(:,n+1:end), tspan(tspan < 0.3));

% Control integrals
effort = control_effort(tspan(tspan < 0.3), u, false);

...
%% GNC 1: Multi-impulsive, staging guidance %% 
% This script provides an interface to test the MISG guidance core. 
...
r_t0 = target_orbit.Trajectory(600,1:6);                    % Initial target conditions
...
%% GNC: MISG guidance %%
% Scheme setup
tol = [1e-8 1e-6];      % Convergence toleranes 
N = 100;                % Number of impulses
method = 'MPC';         % Solver method                                

% Controller scheme
tic
[tspan, St, dV, state] = MISG_control(mu, 1, s0, method, N, tol);  
toc
...
zlabel('$z$');
grid on;
legend('Initial orbit', 'Guidance trajectory', 'Target orbit', 'Location', 'northeast');

...
function [Sc, u, state] = iLQR_control(mu, tf, s0, GNC)
    ...
    dt = 1e-4;                                                                                            % Nondimensional time step
    ...
    while (GoOn && iter < maxIter)
        ...
        u = u+du;
        for i = 1:size(Sc,1)-1
            F(:,1) = [nlr_model(mu, true, false, false, 'Encke', tspan(i), Sc(i,1:2*n).')]+[zeros(9,1); u(:,i)];
            aux = [Sc(i+1,1:n).'; Sc(i,n+1:end).'+F(n+1:end,1)*dt];
            F(:,2) = [nlr_model(mu, true, false, false, 'Encke', tspan(i+1), aux(1:2*n))]+[zeros(9,1); u(:,i+1)];

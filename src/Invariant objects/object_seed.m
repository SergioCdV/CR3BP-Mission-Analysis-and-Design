...
function [seed, T] = lyapunov_seed(mu, parameters)
    %Constants 
    rho = 10000;            %Number of pointer per period
...
function [seed, T] = halo_seed(mu, parameters)
    ...
    L = parameters(3);      %Lagrange point number 
    gamma = parameters(4);  %Lagrange point coordinate
    m = parameters(5);      %Number of periods
    
    %Independent variable     
    tau = 0:dtheta:2*pi*m+dtheta;
    
    %Determine some boolean parameters for the halo determination
    if (L == 1) 
        won = 1; 
        primary = 1-mu;
    elseif (L == 2) 
    ...
    c = legendre_coefficients(mu, L, gamma, 4);
...
function [seed, T] = torus_seed(mu, parameters)
    ...
    %Temporal parametrization
    T = (2*pi)/wp;                     %Period of the orbit 
    tspan = linspace(0, n*tf, rho);    %Integration vector

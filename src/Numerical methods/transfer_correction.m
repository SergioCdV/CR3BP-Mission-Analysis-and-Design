...
function [xf, dVf, state] = HOI_transfer(mu, parking_orbit, target_orbit, maxIter, tol, varargin)
    ...
    if (min(distance) < hd)
        range = linspace(1.05*min(distance), hd, 20);                     
    else
        range = linspace(0.95*min(distance), hd, 20);                    
    end
    ...
    while (h_index <= length(range))
        ...
        dVr = St(1,4:5).'-sHalo(4:5);           %Relative velocity to the HOI velocity
        dVhoi = norm(dVr);                      %Velocity module
        beta = atan2(dVr(2),dVr(1));            %Relative angle 
        ds0 = dVr;                              %Relative velocity
        ...
        while ((GoOn) && (iter < maxIter))
            ...
            dvV = norm(ds0)./ds0;
            dbV = (ds0(1)^2+ds0(2)^2)./[-ds0(2); ds0(1)];
            dvV(3) = 0;
            dbV(3) = 0; 
            ...
            beta = beta + dV(2,iter);                                   %Relative angle
            dVhoi = dVhoi + dV(1,iter);                                 %Relative module
            ut = [cos(beta); sin(beta)];                                %Unit direction vector
            ds0 = dVhoi*ut;                                             %Velocity change
            s0(4:5) = ds0+sHalo(4:5);                                   %Initial conditions

...
function [r] = ZeroVelocitySurface(mu, C, display_flag)
    % Sanity checks 
    ...
    [X, Y, Z] = meshgrid(d, d, d);                % Sampled configuration space
    ...
    if (display_flag)
        ...
        view(3)
        hold on
        patch(isosurface(X, Y, Z, Jc, C), 'FaceColor', 'red', 'EdgeColor', 'none');

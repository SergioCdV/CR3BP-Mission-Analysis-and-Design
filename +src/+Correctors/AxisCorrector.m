...
%% Axis Differential Corrector %% 
% This class implements a differential corrector object for orbits symmetrix around the X axis %

classdef AxisCorrector < src.Correctors.DiffCorrector    
    methods
        % Basic constructor
        function [obj] = AxisCorrector()
            % Parent object
            obj@src.Correctors.DiffCorrector( 3, 2 )

            % Constraint function 
            obj.ComputeConFunction = @(Orbit, ds)obj.AxisConstraint(Orbit, ds);
        end
    end

    methods (Static)
        % Pre-defined constraint function 
        function [e, M, Orbit] = AxisConstraint(Orbit, ds)
            % Apply the computed correction 
            Orbit.State([1 5:6],1) = Orbit.State([1 5:6],1) + ds;
            ...
            % Compute the error
            e = [y(3,end); y(4,end)];                       % Z and Vx at the crossing must be 0
            ...
            M = [Phi(3:4,1) Phi(3:4,5) Phi(3:4,6)] - [y(6,end); F(4,1)] / y(5,end) * [Phi(2,1) Phi(2,5) Phi(2,6)];
        end
    end
end
